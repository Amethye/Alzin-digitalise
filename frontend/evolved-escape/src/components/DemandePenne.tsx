...
const DemandePenne: React.FC = () => {
  ...
  return (
    <div className="mx-auto flex w-full max-w-lg flex-col gap-4 rounded border bg-white px-4 py-5 shadow sm:px-6">
      <h1 className="text-xl font-bold text-bleu sm:text-2xl">Demande de Penne</h1>
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <button
          type="submit"
          className="mt-1 rounded bg-bleu px-4 py-2 text-white transition hover:bg-blue-700"
        >
          Envoyer la demande
        </button>
      </form>

      {message && <p className="mt-2 text-sm text-green-600 sm:text-base">{message}</p>}
    </div>
  );
};
