...
import LogoCapUrl from "@images/LogoCap.svg?url";
...
export default function Nav() {
  ...
  const logoSrc = LogoCapUrl.split("?")[0];
      ...
      <nav className="container mx-auto flex flex-col gap-4 p-4 lg:grid lg:grid-cols-[1fr_auto]">
        <div className="flex items-center justify-between gap-4 lg:col-span-2">
          ...
          >
            ...
            />
            <span className="text-base font-semibold leading-tight text-bleu sm:text-lg">
              <span className="lg:hidden">CAP</span>
              <span className="hidden lg:inline">Centrale d'achat polytech</span>
            </span>
          </a>
          ...
          <div className="flex items-center gap-3">
              ...
              {me ? (
                <div className="relative inline-block">
                  <button
                    ...
                    className="flex items-center gap-1 rounded-md px-2 py-1 focus:outline-none focus:ring-2 focus:ring-bleu focus:ring-offset-2"
                  >
                    Connecté
                        ...
                        {userMenuLinks.map((link) => (
                          <li key={link.href}>
                            <a
                              ...
                              className="block px-4 py-2 text-bleu hover:bg-blue-50"
                            >
                              {link.label}
                            </a>
                          </li>
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-md border border-bleu px-3 py-2 text-sm font-semibold text-bleu shadow-sm transition hover:bg-bleu hover:text-white lg:hidden"
            >
              <svg
                className="h-5 w-5"
                viewBox="0 0 24 24"
        ...
        <>
          ...
          >
            <div className="flex items-center justify-between border-b px-4 py-4">
              <span className="text-base font-semibold text-bleu">Menu</span>
              <button
                type="button"
                onClick={toggleMobileMenu}
                className="rounded-md p-2 text-bleu transition hover:bg-blue-50"
              >
                <svg
                  className="h-6 w-6"
                  viewBox="0 0 24 24"
            ...
            <div className="border-b px-4 py-4 text-sm text-gray-700">
              {me ? (
                <div>
                  <p className="font-semibold text-bleu">{me.identifiant}</p>
                  ...
                  <div className="mt-3 space-y-2">
                    {userMenuLinks.map((link) => (
                      <a
                        ...
                        href={link.href}
                        onClick={handleNavItemClick}
                        className="block rounded-md border border-bleu px-3 py-2 text-center text-bleu transition hover:bg-blue-50"
                      >
                        {link.label}
                      </a>
                    ))}
                ...
                <li key={item.href}>
                  {item.label === "Site Fédé" ? (
                    <a
                      href={item.href}
                      onClick={handleNavItemClick}
                      className="flex w-full items-center justify-center rounded-lg border-2 border-bleu bg-bleu px-4 py-2 text-white transition duration-150 hover:bg-blue-50 hover:text-bleu"
                    >
                      {item.label}
                    </a>
                  ) : (
                    <a
                      href={item.href}
                      onClick={handleNavItemClick}
                      className="relative block px-1 py-1 text-center after:absolute after:bottom-0 after:left-1/2 after:z-50 after:h-[3px] after:w-0 after:-translate-x-1/2 after:rounded-full after:bg-bleu after:duration-500 hover:after:w-full"
                    >
                      {item.label}
                    </a>
                  )}
